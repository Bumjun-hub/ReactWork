/*
  * useEffect()
    : 컴포넌트의 lifecycle을 관리하는 리액트 Hook
      컴포넌트가 생성될 때 (mount) -> update될 때 -> 삭제될때 (unmount)
      이런 lifectcle에서 렌더링 될 때마다 특정작업(side Effect)을 실행함.
      - side effect : 컴포넌트가 렌더링 된 이후에 비동기로 처리되어야 하는 부수적인 효과들 
    
  * 사용
      타이머, mount가 된 후 서버에서 데이터를 백그라운드로 가져올 때 ...

  
  * 사용법
    useEffect((매개변수 (없으면 x))=>{실행할 내용},[배열]);
